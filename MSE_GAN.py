...
def load_train_data():
    ...
    ad_matrix = np.load('train_data\\3000_adj.npy')
    return pos, ad_matrix
PLT_SAVE_FILE = 'generator_training\\mse_gan\\0905_d1_d2'
...
def generator_model():
    ...
    model.add(tf.keras.layers.Dense(30 * 2 , use_bias=False, activation='tanh'))    # (n, 36)
    model.add(tf.keras.layers.BatchNormalization())
...
def discriminator_model():
    ...
    model.add(tf.keras.layers.Dense(30, use_bias=False))
    model.add(tf.keras.layers.BatchNormalization())
    model.add(tf.keras.layers.LeakyReLU())

    model.add(tf.keras.layers.Dense(8, use_bias=False, activation='tanh'))
    # model.add(tf.keras.layers.BatchNormalization())
    # model.add(tf.keras.layers.LeakyReLU())
