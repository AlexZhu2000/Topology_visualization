...
class DeepWalk:
	...
	def train(self, **kwargs):
		...
		# 将iter改为epochs，size 改为vector_size
...
from generate_topo import READ_json_file as READ_JSON
JSON_PATH = r'F:\zzh\pycharm_program\TOPO_visualazition\json\1.json'
if __name__ == '__main__':
	G = READ_JSON(JSON_PATH )

	D = DeepWalk(G, walk_deep=30, walk_numbers=30, embeding_size=2)
	model = D.train()
	embeddings = D.embeddings()
	print(embeddings)
	color_map = []
	X, Y = [], []
	two = ['100,', '19,', '20,', '21,', '22,', '80,', '58,', '69,', '72,', '18,', '53,', '73,']
	three = ['5,', '11,', '14,', '27,', '50,', '23,', '71,', '31,']
	for node, xy in embeddings.items():
		if node == '15,' or node == '24,':
			color_map.append('r')
		elif node in two:
			color_map.append('g')
		elif node in three:
