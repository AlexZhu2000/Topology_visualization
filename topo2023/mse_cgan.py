...
def load_train_data():
    ...
    adj = np.load('train_data/tp100_nodes_719_adj_padding.npy', allow_pickle=True)
    chess_cate = np.load('train_data/tp100_chessboard_pos.npy', allow_pickle=True)
...
print(POS.shape, ADJACENCY.shape)
# MAX = np.max(POS)
...
def generator_model():
    ...
    # (n, 110 * 111)
    model.add(Conv2D(16, kernel_size=(5, 5), activation='relu', input_shape=(110, 111, 1)))
    ...
    model.add(tf.keras.layers.Dense(68640, use_bias=False))    # (n, 30)
    model.add(tf.keras.layers.BatchNormalization())
    model.add(tf.keras.layers.LeakyReLU())
    model.add(tf.keras.layers.Reshape((60, 1144)))                                     # (n, 18, 2)
...
def discriminator_model():
    ...
    # model.add(tf.keras.layers.Dense(32768, use_bias=False))
    # model.add(tf.keras.layers.BatchNormalization())
    # model.add(tf.keras.layers.LeakyReLU())
    #
    # model.add(tf.keras.layers.Dense(4096, use_bias=False))
    # model.add(tf.keras.layers.BatchNormalization())
...
def generator_loss(fake_out, constrain_pos, ad, real_pos):
    ...
    s = topo_evaluate_batch(pos_xy, ad)
    #print(tf.ones(2).dtype)
    mse_loss = MSE(s, 3. * tf.ones_like(s))
...
def constrain_linear(gen_pos, adj):
    ...
    for i in range(adj.shape[0]):
        ...
        ad = adj[i]
        # print(ad.shape, '***********')
        for node, adj_label in enumerate(ad):
            if adj_label[-1] == 1:
                #如果为根节点，对应Mask设置为1
...
def train(train_datasets, epochs):
    for i in range(epochs):
        ...
        generator.save(f'{PLT_SAVE_FILE}/epoch{i}mse_generator.h5')
        discriminator.save(f'{PLT_SAVE_FILE}/epoch{i}mse_discriminator.h5')
        print(f'EPOCH: {i}...\n')
